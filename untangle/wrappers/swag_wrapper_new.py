...
class SWAGWrapper(DistributionalWrapper):
    ...
    def _sample_and_store_params(self, train_loader, fraction):
        ...
        self._set_bn_stats(train_loader=train_loader, fraction=fraction)
        self._state_dicts.append(
            deepcopy({k: v.cpu() for k, v in self.model.state_dict().items()})
        )

    def _set_model(self, model_index: int):
        """Loads a model from cache based on the index."""
        if model_index < 0 or model_index >= self.num_models:
            msg = "Index out of bounds"
            raise ValueError(msg)

        state_dict = self._state_dicts[model_index]
    ...
    @torch.no_grad()
    def _set_bn_stats(self, train_loader, fraction=None):
